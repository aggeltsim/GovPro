...
public class GovProGui {
       ...
       */
        ...
        case 2 -> {
          ...
          if (result == JOptionPane.OK_OPTION) {
            try {
              int selectedIndex = codeCombo.getSelectedIndex() + 1; // +1 because 
              // data[0] is the header
              String input = newValueField.getText().replace(",", "."); // Convert,
              //  to . for BigDecimal
        ...
        default -> 
            System.out.println("No action corresponds here");
      }
    });
  ...
  private static final String[] expenseCategories = { "Employee Benefits", "Social Benefits",
    "Transfers", "Purchases of goods and services", "Subsidies", "Interest", "Other expenses",
  ...
  private static void displayBudget(
        int year, BudgetDataManager.BudgetType type, JTextArea output) {
    ...
    output.append("\n--- " + year 
        + " " + (type == BudgetDataManager.BudgetType.INCOME ? "INCOMES" : "EXPENSES") + " ---\n");
   ...
   *
   * <p>This method serves as an auxiliary or legacy prediction UI
   ...
   * </p>
   *
   * <p>Index 0 contains column headers.
   * </p>
   */
  static Object[][] data = {
    {"Code", "Name", "Amount"}, // 0
